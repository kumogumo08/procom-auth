...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  isOwnPage = session.loggedIn && session.username === usernameFromURL;
...
document.getElementById('saveProfileBtnTop')?.addEventListener('click', saveProfileAndEventsToServer);
document.getElementById('saveProfileBtnBottom')?.addEventListener('click', saveProfileAndEventsToServer);

// 🔽 写真保存ボタンにイベント登録
document.getElementById('savePhotosBtn')?.addEventListener('click', savePhotosToServer);
...
container.innerHTML =     <blockquote class="instagram-media" data-instgrm-permalink="${url}" data-instgrm-version="14"></blockquote>
 ;
if (window.instgrm) {
